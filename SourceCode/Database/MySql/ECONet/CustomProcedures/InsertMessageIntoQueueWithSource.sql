USE `econet`;
DROP procedure IF EXISTS `InsertMessageIntoQueueWithSource`;

DELIMITER $$
USE `econet`$$
CREATE PROCEDURE `InsertMessageIntoQueueWithSource` (
...
BEGIN
	...
	If d_RowCount = 0 and p_EquipmentId <> 0 Then
		...
		VALUES
		(p_MessageToken, d_UserLocationMacId, d_MACUnpairedId, p_EquipmentId, p_MessageType, p_SourceAddress,
