...
DROP PROCEDURE IF Exists WAPI__ProductLiteratureDescription__Get;

DELIMITER $$
CREATE PROCEDURE WAPI__ProductLiteratureDescription__Get(
    IN prodID varchar(64),
	IN apiKey varchar(64),
...
BEGIN
	...
	SELECT pld.ProductID, pld.SeqNumber, pld.Description
	  ...
	  from ru_productliteraturedescription pld
		...
		INNER JOIN ru_apiaccesskey apik ON apik.AccessKey = apiKey
		INNER JOIN ru_organizationbrand ob ON ob.OrganizationID = apik.KeyOwner
		INNER JOIN rdb_brand b ON b.BrandID=p.BrandID and p.BrandID=ob.BrandID
