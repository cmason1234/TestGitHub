...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[GetCanadaInvoices]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[GetCanadaInvoices]
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[GetCanadaInvoices] 
...
BEGIN
    ...
    WHERE mo.personID = @PersonID and 
		  ...
		  mo.OrganizationID = c.OrganizationId and 
		  c.ClaimSectionCategoryID = csc.ClaimSectionCategoryID and 
		  (c.MFGInvoice is null or c.MFGInvoice = '' ) and 
		  c.totalClaim < 0  and 
		  c.AllocatedToOrganizationId = o2.OrganizationID and 
		  o.OrganizationID  = do.OrganizationID and 
		  do.CountryId in (select countryid from rheemusers..country where country='Canada') 
