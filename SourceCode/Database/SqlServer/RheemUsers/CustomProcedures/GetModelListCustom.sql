...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[GetModelListCustom]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[GetModelListCustom]
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[GetModelListCustom] 
...
SELECT distinct a.ProductID,a.Market,a.BrandID,CASE WHEN LEN(a.Model)=0 THEN 'NO__MODEL__NAME' WHEN LEN(a.Model)>0 THEN a.Model END AS Model,
...
FROM rheemusers..Product a
    ...
    LEFT OUTER JOIN RheemUsers..ProductCertification pc ON a.ProductID=pc.ProductID
    LEFT OUTER JOIN RheemUsers..Certification ct ON pc.CertificationID=ct.CertificationID
...
WHERE (@BrandID = CAST(CAST(0 as BINARY) as UNIQUEIDENTIFIER) OR a.BrandID=@BrandID) and
      ...
      (LEN(@Model) = 0 OR a.Model like '%' + @Model + '%') and
      (LEN(@Badge) = 0 OR ct.DisplayName like '%' + @Badge + '%')
	  Order by Model
