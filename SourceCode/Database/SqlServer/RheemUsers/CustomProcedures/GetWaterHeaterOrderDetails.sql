...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[GetWaterHeaterOrderDetails]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[GetWaterHeaterOrderDetails]
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[GetWaterHeaterOrderDetails] 
...
BEGIN
	...
	select ship_to_name, ship_to_address1, ship_to_address2, ship_to_city, ship_to_state, ship_to_zip,
	  ...
	  max(schedule_ship_date) as schedule_ship_date, ord_header_status, po_date 
	from WHOrders who
	  WHERE who.legacy_customer_number = @CustomerNumber AND
